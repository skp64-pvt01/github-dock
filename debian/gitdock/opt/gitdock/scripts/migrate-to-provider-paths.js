...
function migrate() {
  ...
  for (const m of needsMigration) {
    ...
    for (const repoName of m.repos) {
      ...
      // Skip the GitDock project source itself (has package.json with name "gitdock")
      const pkgPath = path.join(oldPath, "package.json");
      try {
        if (fs.existsSync(pkgPath)) {
          const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
          if (pkg.name === "gitdock") {
            warn("Skipping GitDock source project: " + oldPath);
            skipped++;
            continue;
          }
        }
